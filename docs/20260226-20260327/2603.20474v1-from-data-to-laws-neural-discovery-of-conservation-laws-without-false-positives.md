...
tags: ["query:xrlsr"]
score: 10.0
evidence: 守恒定律的神经发现及使用PySR进行符号提取
tldr: 本研究提出NGCG，一种用于从数据中自动发现守恒律的神经符号流水线。该方法通过解耦动力学学习与不变性发现，利用多重启方差最小化器学习近常数潜在表示，并结合符号提取技术生成闭式表达式。NGCG在包含哈密顿系统、混沌系统和偏微分方程的基准测试中表现卓越，实现了零误报率和高发现率，且对噪声具有鲁棒性，为科学发现提供了高效且可解释的工具。
selection_source: fresh_fetch
motivation: 旨在解决现有守恒律发现方法在处理非多项式不变量、局部极小值以及混沌系统误报等方面的局限性。
method: 采用解耦动力学学习与不变性发现的策略，结合多重启方差最小化、符号提取及严格的恒定性过滤机制。
result: "在九个动力系统基准测试中，NGCG实现了100%的发现率和零误报，且在噪声鲁棒性和运行效率上显著优于现有基准。"
conclusion: NGCG通过结合神经学习与符号回归，为复杂动力系统提供了一种高精度、可解释且无误报的守恒律发现新方案。
---

## 摘要
守恒律是理解动力系统的基础，但由于参数变化、非多项式不变量、局部极小值以及混沌系统中的假阳性，从数据中发现守恒律仍然具有挑战性。我们提出了 NGCG，这是一种神经符号流水线，它将动力学学习与不变量发现解耦，并系统地解决了这些挑战。一个多重启动的方差最小化器学习近乎常数的隐式表示；针对特定系统的符号提取（多项式 Lasso、对数基 Lasso、显式 PDE 候选和 PySR）产生闭式表达式；严格的恒定性门控和多样性过滤器消除了伪定律。在包含哈密顿和耗散 ODE、混沌以及 PDE 的九个多样化系统基准测试中，NGCG 在所有四个具有真实守恒律的系统上实现了持续发现（DR=1.0, FDR=0.0, F1=1.0），其恒定性比最佳基准低两到三个数量级。它是唯一在 Lotka--Volterra 系统上取得成功的方法，并且在所有五个没有不变量的系统上正确地输出了无定律。大量实验证明了其对噪声（σ= 0.1）的鲁棒性、样本效率（50--100 条轨迹）、对超参数的不敏感性，以及每个系统不到一分钟的运行时间。帕累托分析表明，该方法提供了一系列候选表达式，允许用户在复杂性和恒定性之间进行权衡。与之前的基于数据的守恒律发现方法相比，NGCG 实现了强大的性能，兼具高精度和可解释性。
