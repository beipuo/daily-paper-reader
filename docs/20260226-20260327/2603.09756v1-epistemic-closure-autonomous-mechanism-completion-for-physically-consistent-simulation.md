...
tags: ["query:xrlsr"]
score: 8.0
evidence: 用于提取控制方程和物理定律的神经符号生成代理
tldr: 针对大语言模型在科学发现中因忽视文献隐含假设而导致的“物理幻觉”问题，本文提出了一种神经符号生成智能体。该智能体通过将物理定律模块化为本构技能，并利用思维链推理进行自主验证与机制补全。在低渗透砂岩热加压模拟中，该智能体成功识别出隐含的排水机制并补全了达西流模型，避免了传统方法的错误预测，实现了物理一致的仿真。
selection_source: fresh_fetch
motivation: 解决大语言模型在提取科学方程时因无法识别文献中隐含的物理假设而导致的物理幻觉问题。
method: 开发了一种神经符号生成智能体，通过模块化本构技能和无量纲标度分析，利用思维链推理自主验证并补全物理机制。
result: 在砂岩热加压挑战中，智能体准确识别出系统处于排水状态并补全了缺失的达西流耗散机制，预测结果与实验事实一致。
conclusion: 该研究确立了AI智能体作为认识论伙伴的新范式，使其能够理解并修正科学数据中嵌入的理论假设。
---

## 摘要
大语言模型（LLMs）在科学发现中的集成目前受到“隐式上下文”问题的阻碍，即从文献中提取的控制方程包含不可见的物理热力学假设（例如，不排水条件），而标准生成模型无法识别这些假设。这导致了“物理幻觉”：生成了语法正确但忠实执行物理无效定律的求解器。在此，我们介绍了一种神经符号生成智能体（Neuro-Symbolic Generative Agent），它在传统数值引擎之上充当认知监督者。通过将物理定律封装为模块化的“本构技能”（Constitutive Skills）并利用潜在的内在先验，该智能体采用思维链（Chain-of-Thought）推理工作流来自主验证、剪枝和补全物理机制。我们在低渗透砂岩的热加压挑战中展示了这一能力。虽然标准的文献检索基准通过盲目采用僵化的“不排水”简化，错误地预测了灾难性的材料失效，但我们的智能体通过无量纲标度分析，自主识别出系统运行在排水状态（德博拉数 De << 1）。因此，它归纳性地补全了满足边界约束所需的缺失耗散机制（达西流），预测了与实验事实一致的稳定应力路径。这项工作建立了一个范式，使 AI 智能体超越了代码助手的角色，成为能够对科学数据中嵌入的理论假设进行推理和修正的认识论伙伴。
