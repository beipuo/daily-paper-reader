...
tags: ["query:xrlsr"]
score: 9.0
evidence: 利用符号探索自主发现物理定律（PDE）的解析解
...
method: 提出了一种名为LawMind的框架，通过整合结构化符号探索与物理约束评估，仅依据物理定律逐步构建并验证有效的解分量。
result: 在涵盖100个权威基准PDE的测试中，LawMind不仅准确还原了所有已知解析解，还发现了此前未见报道的线性与非线性方程新解。
conclusion: 该研究确立了一种由方程驱动自主符号发现的新范式，实现了偏微分方程解析解的系统化推导。
---

## 摘要
偏微分方程（PDEs）编码了基本的物理规律，然而许多重要方程的闭式解析解仍然未知，且通常需要大量的人类洞察力来推导。现有的数值、物理信息和数据驱动方法主要通过数据来逼近解，而不是直接从控制方程中系统地推导符号表达式。在本文中，我们介绍了 LawMind，这是一个规律驱动的符号发现框架，它在不依赖数据或监督的情况下，从偏微分方程及其相关条件中自主构建闭式解。通过将结构化符号探索与物理约束评估相结合，LawMind 仅在控制规律的指导下，逐步组装有效的解组件。在从两本权威手册中提取的 100 个基准偏微分方程上进行的评估显示，LawMind 成功恢复了所有案例的闭式解析解。除了已知解之外，LawMind 还进一步发现了线性和非线性偏微分方程中此前未见报道的闭式解。这些发现建立了一种计算范式，即仅靠控制方程即可驱动自主符号发现，从而实现偏微分方程解析解的系统化推导。
