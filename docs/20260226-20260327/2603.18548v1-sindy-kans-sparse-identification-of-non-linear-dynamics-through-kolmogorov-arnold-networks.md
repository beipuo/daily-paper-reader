...
tags: ["query:xrlsr"]
score: 9.0
evidence: 利用 KAN 进行非线性动力学的稀疏识别以提高可解释性
...
## 摘要
Kolmogorov-Arnold 网络 (KANs) 已作为增强机器学习可解释性的一种潜在途径而出现。然而，从稀疏性或简洁性的角度来看，KANs 学习到的解并不一定具有可解释性。非线性动力学稀疏识别 (SINDy) 是一种互补的方法，能够从数据中学习动力系统的稀疏方程；然而，学习到的方程受限于预定义的函数库。在这项工作中，我们提出了 SINDy-KANs，它同时训练 KAN 和类 SINDy 表示，通过在每个激活函数层面应用 SINDy 来提高 KAN 表示的可解释性，同时保留了深度 KAN 所能实现的函数复合能力。我们将该方法应用于包括动力系统在内的多项符号回归任务，展示了在多种系统中准确发现方程的能力。
