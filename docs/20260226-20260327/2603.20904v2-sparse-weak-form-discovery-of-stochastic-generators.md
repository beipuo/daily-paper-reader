...
tags: ["query:xrlsr"]
score: 8.0
evidence: 利用稀疏识别进行随机微分方程的数据驱动发现
...
conclusion: 该弱形式发现框架为从噪声观测数据中稳健、无偏地提取随机动力学规律提供了一种创新的计算工具。
---

## 摘要
所提出的算法通过将弱形式化应用于随机 SINDy，旨在为随机微分方程（SDE）的发现提供一种新颖的数据驱动框架。该算法的弱形式化提供了一种对噪声具有鲁棒性的方法，避免了传统的使用有限差分进行含噪声导数计算。另一个创新之处在于采用了空间高斯测试函数来替代时间测试函数，其中核权重 $K_j(X_{t_n})$ 的使用保证了期望的无偏性，并防止了时间测试函数中常见的结构回归偏差。该框架将 SDE 辨识问题转化为两个基于 SINDy 的线性稀疏辨识问题。我们在三个 SDE 上验证了该算法，恢复了所有活跃的非线性项，其系数误差低于 4%，平稳密度的全变差距离低于 0.01，且自相关函数在所有三个基准测试中均能忠实地重现真实的弛豫时间尺度。
