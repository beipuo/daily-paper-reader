...
tldr: 本研究针对HVAC设备数字孪生建模中物理方程缺失或过于简化的挑战，提出了一种名为Dyad Model Discovery的半自动化科学机器学习方法。该方法利用遥测数据，通过符号回归自动发现并补充物理模型中的缺失项。通过在运输制冷机组上的应用，展示了该方法如何通过“工程师在环”工作流提升预测性能，实现了首个AI辅助的工程设计流程。
selection_source: fresh_fetch
motivation: 传统的动态模型常因过度简化或缺失物理机制，导致其在校准实际遥测数据时表现不佳。
method: 提出一种半自动化的Dyad Model Discovery方法，利用科学机器学习从数据中提取符号表达式来增强物理方程。
result: 在运输制冷机组数字孪生上的实验证明，该方法能有效利用遥测数据提升模型的预测准确性。
conclusion: 该研究构建了首个AI辅助工程设计工作流，通过人机协作模式优化了复杂系统的建模与设计过程。
---

## 摘要
将动态模型与数据进行校准是构建暖通空调（HVAC）设备、热负荷及控制系统数字孪生的重要步骤。有时，当模型无法与数据校准时，可能的原因是模型做了过多的简化假设并缺失了物理机制。在本文中，我们提出了一种名为 Dyad 模型发现（Dyad Model Discovery）的半自动化方法，该方法可以用从数据中发现的符号表达式来增强模型的物理方程。我们在运输制冷单元的数字孪生上演示了该方法，利用遥测数据进行训练以提高其预测性能。本文提出了一种工程师在环（engineer-in-the-loop）的工作流，该工作流向用户提供建议，用户随后可以接受或拒绝这些建议。据我们所知，这是首个 AI 辅助的工程设计工作流。
