...
tags: ["query:xrlsr"]
score: 9.0
evidence: 利用大语言模型和符号进化进行数据驱动的控制方程发现
tldr: 本研究针对动力系统控制方程的自动发现问题，提出了一种名为LLM-ODE的新型框架。传统遗传算法（GP）在符号搜索空间中效率低下且收敛缓慢，LLM-ODE通过引入大语言模型的生成先验，从精英候选方程中提取模式来引导符号进化。在91个动力系统上的实验表明，该方法在搜索效率、帕累托前沿质量及高维系统扩展性方面均优于传统GP和Transformer模型，为科学发现提供了更高效、准确的数据驱动工具。
selection_source: fresh_fetch
motivation: 传统的遗传算法在发现动力系统方程时存在搜索空间探索效率低、收敛速度慢且易陷入局部最优的问题。
method: 提出LLM-ODE框架，利用大语言模型从精英候选方程中提取模式并引导符号进化，结合了LLM的生成能力与进化算法的探索优势。
result: 在91个动力系统的测试中，LLM-ODE在搜索效率和帕累托前沿质量上均优于传统遗传算法，且在高维系统中表现出更强的扩展性。
conclusion: LLM-ODE通过将大语言模型集成到符号进化过程中，显著提升了动力系统方程发现的效率和准确性。
---

## 摘要
发现动力系统的控制方程是许多科学学科的核心问题。随着实验数据的日益丰富，自动方程发现方法提供了一种极具前景的数据驱动方法，以加速科学发现。在这些方法中，遗传规划（GP）因其灵活性和可解释性而被广泛采用。然而，基于 GP 的方法通常在符号搜索空间的探索效率低下，导致收敛缓慢且解的质量不佳。为了解决这些局限性，我们提出了 LLM-ODE，这是一种大语言模型辅助的模型发现框架，它利用从精英候选方程中提取的模式来引导符号演化。通过利用大语言模型的生成先验，LLM-ODE 在保留演化算法探索优势的同时，产生了更具信息量的搜索轨迹。在 91 个动力系统上的实证结果表明，LLM-ODE 变体在搜索效率和帕累托前沿（Pareto-front）质量方面始终优于传统的 GP 方法。总之，我们的结果表明，与传统的基于 GP 的发现方法相比，LLM-ODE 提高了效率和准确性，并且与线性及仅使用 Transformer 的模型发现方法相比，对高维系统具有更强的可扩展性。
