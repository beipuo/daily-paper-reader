...
tags: ["query:xrlsr"]
score: 10.0
evidence: 利用具有离散符号结构的神经网络架构进行控制方程的符号发现
...
## 摘要
发现控制方程的符号化方法是科学机器学习的一个长期目标，但在可解释性与可扩展学习之间始终存在着根本性的权衡。经典的符号回归方法可以产生显式的解析表达式，但依赖于组合搜索；而神经网络在数据和维度上具有良好的扩展性，但产生的表示却是不透明的。在这项工作中，我们引入了符号 Kolmogorov-Arnold 网络（Symbolic-KANs），这是一种通过将离散符号结构直接嵌入到可训练深度网络中来弥合这一差距的神经架构。Symbolic-KANs 将多元函数表示为应用于学习到的标量投影上的学习单变量基元的组合，并由解析基元库、分层门控和符号正则化引导，这些机制将连续混合逐渐细化为 one-hot 选择。经过门控训练和离散化后，每个激活单元选择一个基元和投影方向，从而产生紧凑的闭式表达式，而无需事后的符号拟合。Symbolic-KANs 还可以作为可扩展的基元发现机制，识别出最相关的解析组件，随后可为稀疏方程学习方法提供候选库。我们证明了 Symbolic-KAN 在数据驱动的回归和逆动力系统任务中能够可靠地恢复正确的基元项和控制结构。此外，该框架还扩展到了偏微分方程的正向和反向物理信息学习，直接从控制约束中产生精确解，同时构建紧凑的符号表示，其选择的基元反映了底层方程的真实解析结构。这些结果使 Symbolic-KAN 成为迈向可扩展、可解释且具有机制基础的控制律学习的一步。
