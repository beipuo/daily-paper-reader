...
title_zh: OMNIFLOW：一种用于泛化科学推理的基于物理的多模态智能体
...
tags: ["query:xrlsr"]
score: 8.0
evidence: 用于物理定律和科学推理的神经符号架构
tldr: 针对大语言模型在处理偏微分方程（PDE）等复杂物理动力学时易产生非物理幻觉的问题，本文提出 OMNIFLOW。这是一种神经符号架构，通过语义-符号对齐机制将高维流张量转化为拓扑语言描述符，并结合物理引导的思维链（PG-CoT）进行推理。该方法无需特定领域微调，在湍流、纳维-斯托克斯方程及天气预报等任务中表现优异，实现了从黑盒拟合到可解释科学推理的转变。
selection_source: fresh_fetch
motivation: 解决大语言模型在处理受偏微分方程约束的连续时空动力学时存在的非物理幻觉及跨领域泛化性差的问题。
method: 提出一种神经符号架构，利用语义-符号对齐机制将物理张量转化为拓扑描述符，并引入包含动态约束注入和迭代验证的物理引导思维链。
result: 在微观湍流、理论流体力学方程及宏观天气预报等多个基准测试中，该模型在零样本泛化和少样本适应任务上显著优于传统深度学习基准。
conclusion: OMNIFLOW 成功将冻结的多模态大模型与基础物理定律相结合，为实现透明且物理一致的通用科学推理提供了新范式。
---

## 摘要
大语言模型（LLMs）展现了卓越的逻辑推理能力，但在处理由偏微分方程（PDEs）支配的连续时空动力学时经常遇到困难，往往导致非物理的幻觉。现有方法通常依赖于昂贵的、特定领域的微调，这严重限制了跨领域泛化能力和可解释性。为了弥补这一差距，我们提出了 OMNIFLOW，这是一种神经符号架构，旨在将冻结的多模态 LLMs 植根于基本物理定律，而无需特定领域的参数更新。OMNIFLOW 引入了一种新颖的语义-符号对齐（Semantic-Symbolic Alignment）机制，将高维流张量投影为拓扑语言描述符，使模型能够感知物理结构而非原始像素值。此外，我们构建了一个物理引导的思维链（PG-CoT）工作流，通过动态约束注入（例如质量守恒）和迭代反思验证来编排推理。我们在一个涵盖微观湍流、理论纳维-斯托克斯（Navier-Stokes）方程和宏观全球天气预报的综合基准上评估了 OMNIFLOW。实证结果表明，OMNIFLOW 在零样本泛化和少样本自适应任务中显著优于传统的深度学习基准。至关重要的是，它提供了透明且物理一致的推理报告，标志着从黑盒拟合到可解释科学推理的范式转变。
