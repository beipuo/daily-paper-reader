...
tags: ["query:xrlsr"]
score: 10.0
evidence: 使用遗传规划进行随机微分方程的符号发现
tldr: 本研究针对传统符号回归仅关注确定性动力学而忽略噪声的问题，提出了一种基于遗传算法的随机微分方程（SDE）符号发现方法。该方法通过极大似然估计同时优化漂移项和扩散项，实现了对随机动力系统可解释数学表达式的自动提取。实验证明，该方法在方程恢复准确性、高维系统扩展性及稀疏采样鲁棒性方面表现优异，并能推广至随机偏微分方程，为噪声环境下的科学发现提供了有力工具。
selection_source: fresh_fetch
motivation: 传统的符号回归通常将噪声视为干扰，而忽略了显式建模随机微分方程中噪声项对提升模型容量和科学理解的重要性。
method: 提出一种基于遗传算法的方法，通过极大似然估计联合优化随机微分方程中的漂移函数和扩散函数。
result: 该方法能够准确恢复控制方程，有效扩展至高维系统，并在稀疏采样和随机偏微分方程任务中展现出强鲁棒性。
conclusion: 本研究将符号回归扩展到了可解释的随机动力系统领域，为自动化科学发现提供了处理复杂噪声动态的新途径。
---

## 摘要
自动化科学发现旨在通过机器学习提升科学理解。该领域的一个核心方法是符号回归，它利用遗传规划或稀疏回归来学习可解释的数学表达式，以解释观测数据。传统上，符号回归的重点在于识别常微分方程。普遍观点认为，噪声只会使确定性动力学的恢复变得复杂。然而，显式地学习随机微分方程中噪声分量的符号函数，可以增强建模能力，增加知识获取，并实现生成式采样。我们提出了一种基于遗传规划的随机微分方程符号化发现方法，通过极大似然估计联合优化漂移函数和扩散函数。我们的结果表明，该方法能够准确恢复控制方程，有效扩展到高维系统，对稀疏采样问题具有鲁棒性，并能推广到随机偏微分方程。这项工作将符号回归扩展到了随机动力系统的可解释性发现，为噪声和动态世界中的科学自动化做出了贡献。
